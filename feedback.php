...
class feedback
{
  ...
  static function check($request)
  {
    ...
    $v->check('title')->is(2);
    $v->check('type')->is('in(mukonin_audit.feedback_type.code)','optional'); 
  ...
  {    
    ...
    $sql = "INSERT into mukonin_audit.feedback(program_id,user_id,partner_id,title,description,type,status_code)
