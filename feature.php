...
class feature
{
  ...
  {
    return select::add_db('select code, description from status where code !="decl"','','','--Select Status--');
  }
  ...
  {    
    ...
    $sql = "INSERT into feature_request(program_id,user_id,partner_id,title,description,priority,status_code)
            VALUES($program_id,$user_id,$partner_id,'$title','$description','med','pend')";
    $db->exec($sql);
    
    list($email_address,$requestor)= $db->read_one("SELECT email_address,concat(first_name,' ' ,last_name) user from user  
                                   where id=$user_id");
    $rows = $db->read("select email_address from user u, partner p, user_role r
  ...
  static function update_priority($request)
  {
    ...
    $sql = "update feature_request set priority='$priority',status_code='$status' where id = $id";
    $db->exec($sql);
  ...
  static function manage_features($request)
  {
   ...
   $program_id = config::$program_id;
    ...
    $sql = "select * from (select fr.id,fr.create_time,full_name,concat(first_name,' ',last_name) requestor,fr.title,fr.description,pr.name priority,s.description status, r.description reason
            ...
            join partner p on p.id = fr.partner_id
            join status s on s.code = fr.status_code
            left join priority pr on pr.code = fr.priority
            left join reason r on r.code = fr.status_reason_code 
  ...
  {      
    ...
    $sql = "select  title,description reason,count(comment) comments
            from feature_request fr
            left join feature_comment fc on fr.id=fc.feature_id
            where fr.id = $id ";
  ...
  static function add_comment($request)
  {
    ...
    $sql = "INSERT into feature_comment(program_id,feature_id,user_id,comment)
           VALUES($program_id,$id,$user_id,'$comment')";
    $db->exec($sql);
    
    $rows = $db->read("select email_address,fr.title from user u, feature_request fr
