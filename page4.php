...
{  
  ...
  function load_fields($file, $search_paths=array('.'))
  {
    $i = 0;
    ...
    $this->fields_stack = array_merge($this->fields_stack, $$fields);
  }

  function load()
  {
    $search_paths = array('../common', '.');
    $this->load_fields('controls.yml', $search_paths);
    $this->load_fields('fields.yml', $search_paths); 

    if ($this->path[0] == '/') $this->path = substr ($path, 1);
    $this->path = explode('/', $path);
    $this->load_fields($this->path[0] . "yml", $path);
  }
  

  function explode_field($code, &$field = array())
  {
    foreach ($this->fields_stack as $fields) {
      $base_field = $fields[$code];
      if (!isset($base_field)) continue;
      $field = merge_options($field, $base_field);
    }
    
    $type = array();
    $rendering = $this->action == 'read';
    if (is_assoc($field)) {
      foreach($field as $key=>&$values) {
        if ($key == 'type') 
          $type = $this->explode_field($values);
        else
          $this->explode_field($key, $values);
      }
      $field = merge_options($field, $type);
      return $field;
    }
    
    foreach ($field as $key=>&$value) {
      if (is_string($value)) {
        $value = $this->explode_field($value);
        continue;
      }
      
      if (!is_assoc($value)) continue;
      
      if ($key == 'type') {
        $type = explode_field($value);
        if ($rendering)
          $this->types[$code] = $type;
        continue;
      }
      
      list($code, $values) = assoc_element($value);
      $this->explode_field($code, $values);
      $merged_value = merge_options($type, $values);
      
      
      $access = $merged_value['access'];
      if (!isset($access)) continue;
      
      if (!is_array($access)) $access = explode (',', $access);
      
      $allowed = array_intersect($this->user_roles, $access);
  ...
  function follow_path($path, $field)
  {
    ...
    if (!$found)
      throw new Exception("Invalid path step $step");
  }
  ...
  function merge_known(&$value, $known)
  {
    $known_value = $known[$key];

    if (!is_array($value)) {
      if (!$known_value) return;
      $value = $known_value;
      return;
    }
    
    if ($known_value) 
      $value = merge_options ($known_value, $value);
  }
  ...
  function expand_contents(&$parent, $known)
  {
    ...
    $result = array();
    $reserved = array('sql', 'template', 'attr','valid');
    $found = array();
  ...
  function action()
  {
    ...
    if (!is_null($validate) && $validate != 'none') {
      $fields = $this->fields[$this->page];
      log::debug_json("UNEXPANDED", $fields);
      log::debug_json("FIELDS", $this->fields);
      $this->expand_field($fields, $this->fields, array('valid'));
      log::debug_json("EXPANDED", $fields);
